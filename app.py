...
def extract_source_parts(source: str) -> tuple[str, str, str]:
    ...
    if not (style_match and body_match and script_match):
        raise RuntimeError("The original HTML structure is missing its style, body, or script block.")
...
def build_frontend(source: str) -> tuple[str, str, str]:
    ...
    # The original page is intentionally kept as the source of truth.
    # Only the integration boundary is changed: DOM lookups are rooted at
    # the Streamlit V2 component, and the old Claude-only submission layer
    # is replaced with a Streamlit trigger.
    css = (
        ...
        + css
        + "\n"
        + "/* Streamlit host reset. The questionnaire itself remains the original page. */\n"
        + "[data-testid=\"stMainBlockContainer\"]{max-width:none!important;padding-left:0!important;padding-right:0!important;}\n"
        + ".twi-streamlit-root{width:100%;min-width:0;}\n"
    )

    # Remove the original Claude DB / Claude Downloads integration.
    backend_start = original_js.find("const toastEl = document.getElementById(\"toast\")")
    render_start = original_js.find("render(); update(); watchChapters();")
    if backend_start < 0 or render_start < 0:
        raise RuntimeError("Could not locate the original submission integration in the HTML.")
    core_js = original_js[:backend_start] + original_js[render_start:]
    ...
    # The original script used document.body only for its standalone Markdown
    # download path. Keep that browser download behavior, but make the button
    # visible in Streamlit as well.
    core_js += r'''
    ...
    dlBtn.onclick = () => {
      const slug = (state.client || "answers").replace(/[^\w\- ]+/g, "").trim().replace(/\s+/g, "-") || "answers";
    ...
    if(data?.send_status === "success") {
      ...
      statusEl.textContent = "Answers sent to Another Idea. You can keep editing and send again.";
      toast("Answers sent.");
    } else if(data?.send_status === "error") {
      sendBtn.disabled = false;
      statusEl.textContent = "Couldn’t send. Check the Streamlit SMTP secrets and try again.";
    }
'''
...
export default function(component) {
  ...
  const initialized = root.dataset.twiInitialized === "1";
  if (!initialized) root.dataset.twiInitialized = "1";

""" + core_js + "\n}\n"

    # Keep the original body markup unchanged. A wrapper is added only so the
    # Streamlit component has a stable root without altering the page structure.
...
def send_email(payload: dict) -> None:
    ...
    email_to = st.secrets.get("EMAIL_TO", "gauravgandhi@anotheridea.in")
